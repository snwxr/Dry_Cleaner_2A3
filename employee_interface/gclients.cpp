...
bool Gclients::ajouterClient()
{
    ...
    query.prepare("insert into clients(cin,nom_c,prenom_c,sexe_c,date_naissance,reduction_c,ADRESSE_C) values(:cin, :nom, :prenom, :sexe, :daten, :reduc, :adresse)");
    ...
    query.bindValue(":adresse",AdresseC);
    return query.exec();
}
...
bool Gclients::modifierClient(int cinnn, QString nom, QString prenom, QString sexe, QString date, QString adresse)
{
    QSqlQuery query; //nasn3ou l requette
   ...
   // }
    ...
    //prepare()prend la requete en param pour la preparer a l'execution
    query.prepare("update clients set nom_c=:nom, prenom_c=:prenom, sexe_c=:sexe, DATE_NAISSANCE=:daten,ADRESSE_C=:adresse  where cin = :cin");
...
QString Gclients::CalculReduction()
{
    ...
    if(test)
    {
       return "L'acces à la reduction est à jour";
